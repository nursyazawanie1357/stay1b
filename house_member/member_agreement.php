...
// Fetch agreement details for the house member
$query_agreement = "
    SELECT 
        ta.booked_house_id,
        lh.house_number,
    ...
    LEFT JOIN 
        house_group hg ON hg.booked_house_id = ta.booked_house_id
    LEFT JOIN 
        housemate_role hr_member ON hg.housemate_id = hr_member.housemate_id
    LEFT JOIN 
        tenant tm ON hr_member.tenant_id = tm.tenant_id
    WHERE 
        ta.booked_house_id = (
            SELECT hb.booked_house_id 
            FROM house_group hg
    ...
    GROUP BY 
        ...
        error_log("Agreement Details: " . print_r($agreement_details, true));
...
function generateAgreementPDF() {
    ...
    let membersList = "<?= htmlspecialchars($agreement_details['members_list'] ?? 'No members available') ?>";
    ...
    // Remove the leader's name from the members list
    membersList = membersList.split(', ').filter(member => member !== leaderName).join(', ');
    ...
    doc.text(doc.splitTextToSize(membersList || 'No members available', pageWidth - 2 * textMargin), textMargin + 10, 110);
                ...
                <section class="agreement-details-section">
                    <?php if ($agreement_details): ?>
                        <div class="agreement-details">
                            ...
                            <p class="agreement-detail"><strong>House Members:</strong> 
    <?php 
    $membersList = explode(', ', $agreement_details['members_list'] ?? ''); 
    $filteredMembers = array_filter($membersList, function($member) use ($agreement_details) {
        return $member !== $agreement_details['leader_name'];
    });
    echo htmlspecialchars(implode(', ', $filteredMembers)) ?: 'No members available';
    ?>
</p>
