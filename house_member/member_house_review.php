...
// Get the house the user is associated with from the house_group table
$stmt = $conn->prepare("SELECT hb.house_id FROM house_group hg
...
// Fetch all houses for filter options
$houses = [];
...
if (isset($_GET['delete'])) {
    ...
    $stmt->close();
    header("Location: member_house_review.php"); // Redirect to avoid resubmission
    exit();
}
...
$reviews = [];
$query = "SELECT hr.review_id, hr.review_content, hr.created_at, hr.reviewed_by, hr.rating, lh.house_number
    ...
    <main>
        ...
        <section class="reviews-section">
            ...
            <div class="review-card">
                ...
                <div class="rating">
                    <?php for ($i = 1; $i <= 5; $i++): ?>
                    <span class="star <?php echo $i <= $review['rating'] ? 'selected' : ''; ?>" data-value="<?php echo $i; ?>">&#9733;</span>
                    <?php endfor; ?>
                </div>
