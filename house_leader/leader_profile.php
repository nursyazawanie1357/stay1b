...
include 'leader_sidebar.php'; // Include the leader sidebar PHP file
...
$query_leader = "SELECT tenant.tenant_full_name, tenant.tenant_email, tenant.tenant_whatsapp, tenant.tenant_picture 
                 FROM housemate_role
...
$stmt->execute();
$stmt->bind_result($leader_full_name, $leader_email, $leader_whatsapp, $leader_picture);
$stmt->fetch();
$stmt->close();
    ...
    if (!preg_match('/^[0-9]{10,15}$/', $new_whatsapp)) {
        ...
        if (!empty($_FILES['tenant_picture']['name'])) {
            ...
            if (move_uploaded_file($uploaded_file['tmp_name'], $target_path)) {
                $leader_picture = 'assets/profile-pictures/' . $file_name;
            }
        }

        // Update leader details in the database
        $query_update = "UPDATE tenant SET tenant_email = ?, tenant_whatsapp = ?, tenant_picture = ? WHERE tenant_id = (SELECT tenant_id FROM housemate_role WHERE housemate_id = ? AND house_role = 'leader')";
        $stmt = $conn->prepare($query_update);
        $stmt->bind_param("sssi", $new_email, $new_whatsapp, $leader_picture, $housemate_id);

        if ($stmt->execute()) {
            header("Location: leader_profile.php?success=1");
            exit();
        } else {
...
<head>
    ...
    <title>Leader Profile - Stay1B</title>
...
<body>
    <div class="dashboard-container">
        ...
        <main>
            ...
            <!-- Profile Edit Form -->
            <form action="leader_profile.php" method="POST" enctype="multipart/form-data" class="profile-edit-form">
                <img id="profile-picture-preview" class="profile-picture-preview" src="../<?= htmlspecialchars($leader_picture) ?>" alt="Profile Picture Preview">

                <label for="tenant-email">Email:</label>
                <input type="email" id="tenant-email" name="tenant_email" value="<?= htmlspecialchars($leader_email) ?>" required>

                <label for="tenant-whatsapp">Contact Number:</label>
                <input type="text" id="tenant-whatsapp" name="tenant_whatsapp" placeholder="Enter contact number (10-15 digits)" value="<?= htmlspecialchars($leader_whatsapp) ?>" required>
